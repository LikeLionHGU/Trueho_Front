...
// 5) 배열 값 & 사진 post 하는 함수
  ...
  const postAllDataSequentially = async (e) => {
    ...
    try {
      // (1) 일반 데이터 먼저 전송
      await axios.post(`${process.env.REACT_APP_HOST_URL}/user/edit`, data, {
    ...
    if (uploadFile) {
      ...
      await axios.post(`${process.env.REACT_APP_HOST_URL}/user/image`, formData, {
...
// 7) 사용자 프로필 정보 불러오기
  ...
  const getProfile = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_HOST_URL}/user/edit`);
      console.log("profile 수정 함수 실행 확인");
...
// 9) hansum, showing 값 불러온거로 기본값 버튼 눌리게 세팅
  useEffect(() => {
    // if(profile.hansum===1){
    //   setData({
    //     ...data,
    ...
    if (Object.keys(profile).length > 0) {
      setData(prevData => ({
        ...prevData,
        hansum: profile.hansum === 1 ? "1" : "0",
        showing: profile.showing === 1 ? "1" : "0",
        name: profile.name,
        admission: profile.admission,
        graduation: profile.graduation,
        major: profile.major,
        work: profile.work,
        history: profile.history ?? [{ name: "", detail: "" }],
      }));
    }
  }, [profile]);

// 10) 박스 갯수 history 개수만큼 세팅

  useEffect(() => {
    if (profile.history && Array.isArray(profile.history)) {
      setBoxes(Array.from({ length: profile.history.length }, (_, i) => i + 1));
    }
  }, [profile.history]);
...
{/* 2------------------------------------------------------------ */}
          <div className="newprofile-bottom-container-2 box-row">
            <div className="newprofile-bottom-container-2-pic box-column">
              <img src={uploadImgFile || profile.imgUrl} alt="이미지 미리보기" className="newprofile-bottom-container-2-pic-img" />
              ...
              <div className="newprofile-bottom-container-2-njj box-row">
                ...
                <div className='container_2'>
                  <div className="newprofile-bottom-container-2-njj-input box-column">
                    ...
                    <input placeholder="텍스트를 입력하세요" type="text" name="name" onChange={onChangeInput} value={data.name}/>
                  </div>
                  </div>
              </div>
...
{/* 3------------------------------------------------------------ */}
          <div className="newprofile-bottom-container-3 box-row">
            ...
            <div className='star'><span>*</span></div>
              <input placeholder="현재 직무를 작성해 주세요 (ex. 마케터, 웹디자이너, 학생)" type="text" name="work" onChange={onChangeInput} value={data.work}></input>
            </div>
          </div>
{/* 4------------------------------------------------------------ */}
              ...
              boxes.map((box) => (
                <div key={box} className="newprofile-bottom-container-4-box">
                  <div className="newprofile-bottom-container-4-box-test box-column">
                    <input
                      placeholder="공백 포함 최대 25자 입력하실 수 있습니다"
                      type="text"
                      value={data.history?.[box - 1]?.name || ""}
                    ...
                    <div className="newprofile-bottom-container-4-box-test-in">
                      <input
                        ...
                        onChange={(e) => onChangeHistoryInput(box - 1, "detail", e.target.value)}
                      />
                    </div>
                  </div>
                </div>
